...
router.post("/", async (req, res) => {
    ...
    try {
        const [warehouse] = await connection.query("SELECT id FROM warehouses WHERE id = ?", [warehouse_id]);
        if (warehouse.length === 0) {
            return res.status(400).json({
                message: "Invalid - warehouse does not exist."
            });
        }
        const insertSql = `INSERT INTO inventories (
...
router.route("/:id")
    ...
    try {
        const [result] = await connection.query("DELETE FROM inventories WHERE id = ?", [inventoryId]);
